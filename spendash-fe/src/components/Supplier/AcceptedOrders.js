...
class AcceptedOrderList extends Component {
    ...
    getOrderBySupplier = async () => {
        const {id} = this.state
        let response = await AddReceiptDataService.getOrdersBySupplier('approvedBySupplier',id)
        ...
        return (
            <React.Fragment>
                <Container style={{ marginTop: 25 , border: '2px solid black'}}>
                    ...
                    <h2 style={{textAlign: "center", color: '#2F4F4F'}}> Accepted Orders </h2>
...
export default withRouter(AcceptedOrderList);
